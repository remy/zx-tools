...
const keywords = Object.values(_txt2bas.codes).filter(_ => !['*', 'REM', '$'].includes(_));
const $keywords = Object.values(_txt2bas.codes).filter(_ => _.includes('$')).map(_ => _.replace(/\$/, '\\$'));

_cm.default.defineSimpleMode('basic', {
  ...
  start: [// The regex matches the token, the token property contains the type
  {
    ...
    regex: /(:)\s*(;.*)/,
    token: [null, 'comment'] // eol: true,

  }, {
    regex: /#\w+/,
    sol: true,
    token: ['pragma'] // eol: true,
...
},{"../lib/cm.js":"lib/cm.js","txt2bas":"../node_modules/txt2bas/dist/index.js"}],"lib/save.js":[function(require,module,exports) {
"use strict";
...
},{}],"lib/audio/audio-consts.js":[function(require,module,exports) {
"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.byteAsWord = byteAsWord;
exports.calculateXORChecksum = exports.encode = exports.decode = exports.LOADER = exports.SYN_OFF = exports.SYN_ON = exports.ONE = exports.ZERO = exports.PILOT_DATA_COUNT = exports.PILOT_COUNT = exports.PILOT = exports.LOW = exports.HIGH = exports.toAngularFrequency = exports.asHz = exports.T = exports.SAMPLE_RATE = void 0;
const SAMPLE_RATE = 44100;
exports.SAMPLE_RATE = SAMPLE_RATE;
const T = 1 / 3500000; // pulse width (half a wave cycle) in ms @ 3.5Mhz
...
const toAngularFrequency = hz => hz * 2 * Math.PI; // these are how high and low the pulse value goes in the audio buffer
// 1 and -1 being the extreme max


exports.toAngularFrequency = toAngularFrequency;
const HIGH = 1; // 0.15;
...
const PILOT = 2168;
exports.PILOT = PILOT;
const PILOT_COUNT = 8063;
exports.PILOT_COUNT = PILOT_COUNT;
const PILOT_DATA_COUNT = 3223;
exports.PILOT_DATA_COUNT = PILOT_DATA_COUNT;
const ZERO = 855;
exports.ZERO = ZERO;
const ONE = 2 * ZERO;
exports.ONE = ONE;
const SYN_ON = 667;
exports.SYN_ON = SYN_ON;
const SYN_OFF = 735;
/*
...
exports.SYN_OFF = SYN_OFF;
const LOADER = [0x00, 0x0a, 0x03, 0x00, 0x20, 0xfb, 0x0d, 0x00, 0x14, 0x07, 0x00, 0x20, 0xef, 0x22, 0x22, 0x20, 0xaa, 0x0d, 0x00, 0x1e, 0x0a, 0x00, 0x20, 0xf2, 0x30, 0x0e, 0x00, 0x00, 0x00, 0x00, 0x00, 0x0d];
/**
 * Returns string
 * @param {Uint8Array} a
 */
...
const calculateXORChecksum = array => array.reduce((checksum, item) => checksum ^ item, 0); // words in ZX81 are LSb / little endian


exports.calculateXORChecksum = calculateXORChecksum;

function byteAsWord(byte) {
  return [byte & 0xff, // LSb
  byte >> 8];
}
/**
...
function toLittle(bytes) {
  ...
  const little = dv[`getUint${size}`](0,
  /* little endian data */
  true);
  return new Uint8Array(little.toString(16).padStart(bytes.length * 2, '0').match(/(..?)/g).map(b => parseInt(b, 16)));
}
},{}],"lib/audio/index.js":[function(require,module,exports) {
"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.generateFlatSamples = generateFlatSamples;
...
exports.generateAutoLoaderForScreen = generateAutoLoaderForScreen;
exports.generateScreenBlock = generateScreenBlock;
exports.combineTapImages = combineTapImages;
exports.generateROMImage = generateROMImage;
exports.generateROMImageAsTap = generateROMImageAsTap;
exports.headerAsBytes = headerAsBytes;

var _audioConsts = require("./audio-consts.js");

const calculateXORChecksum = array => Uint8Array.of(array.reduce((checksum, item) => checksum ^ item, 0))[0];

const zeroBit = generateBit(_audioConsts.ZERO);
const oneBit = generateBit(_audioConsts.ONE);
...
function generateFlatSamples({
  output,
  offset,
  pulse,
  value = _audioConsts.HIGH
}) {
  pulse = pulse * _audioConsts.T * _audioConsts.SAMPLE_RATE; // round values to integers
...
function generateBit(pulse) {
  const output = new Float32Array(pulse * 2 * _audioConsts.T * _audioConsts.SAMPLE_RATE + 0.5 | 0);
  generateFlatSamples({
    output,
    offset: 0,
    pulse,
  ...
  const offset = pulse * _audioConsts.T * _audioConsts.SAMPLE_RATE;
  generateFlatSamples({
    output,
    offset,
    pulse,
    value: _audioConsts.LOW
  });
  return output;
}
...
function generateSilence({
  output,
  offset = 0,
  count = _audioConsts.PILOT_DATA_COUNT
}) {
  const pulse = _audioConsts.PILOT; // small bug in my own logic, this produces 8064 half pulses
...
function generatePilot({
  output,
  offset = 0,
  count = _audioConsts.PILOT_COUNT
}) {
  const pulse = _audioConsts.PILOT; // small bug in my own logic, this produces 8064 half pulses
  // this is because the immediately next pulse is a syn on, which
  // is also high, so it doesn't offer any edge detection.
  ...
  for (let i = 0; i < count; i++) {
    offset = generateFlatSamples({
      ...
      offset,
      pulse,
      value: i % 2 === 0 ? _audioConsts.LOW : _audioConsts.HIGH
    });
  }
...
function generateByte({
  ...
  for (let i = 0; i < 8; i++) {
    ...
    const bit = (byte << i & 128) === 128 ? _audioConsts.ONE : _audioConsts.ZERO;
    const buffer = bit === _audioConsts.ONE ? oneBit : zeroBit;
...
function generateBytes({
  ...
  offset = generateFlatSamples({
    output,
    offset,
    pulse: _audioConsts.ZERO,
    value: _audioConsts.HIGH
  });
  return offset;
}

function calculateSampleSize(...pulses) {
  return pulses.reduce((acc, curr) => acc += curr, 0) * _audioConsts.T * _audioConsts.SAMPLE_RATE + 0.5 | 0;
}
/**
 *
 * @param {Object} block
...
function generateBlock({
  ...
  if (param2 === null) {
    if (param1 === 0x4000) param2 = 0x8000;else param2 = data.length;
  } // pre-calculate the count of samples required


  const SILENCE = _audioConsts.PILOT_DATA_COUNT * _audioConsts.PILOT;
  const length = calculateSampleSize(_audioConsts.PILOT_COUNT * _audioConsts.PILOT, _audioConsts.SYN_ON * 2, _audioConsts.SYN_OFF * 2, SILENCE * 2, _audioConsts.PILOT_DATA_COUNT * _audioConsts.PILOT, _audioConsts.ONE * 2, // closing pulses
  19 * 8 * (_audioConsts.ONE * 2), // flag + file type + header (16) + parity
  data.length * 8 * (_audioConsts.ONE * 2), (1 + 1) * 8 * (_audioConsts.ONE * 2) // length, file type, parity
  ); // the +0.5 is just a little "wiggle" room

  const buffer = ctx.createBuffer(1, length + 0.5 | 0, _audioConsts.SAMPLE_RATE);
  const output = buffer.getChannelData(0); // pilot tone
  // eslint-disable-next-line no-unused-vars

  let offset = generatePilot({
    output
  }); // syn on
...
function generateAutoLoaderForScreen({
  ctx
}) {
  return generateBlock({
    ctx,
    data: _audioConsts.LOADER,
    filename: 'tap dot js',
    param1: 10,
    // Autostart LINE Number
    param2: _audioConsts.LOADER.length,
    // Size of the PROG area aka start of the VARS area
    type: 'PROGRAM'
  });
}

function generateScreenBlock({
  ctx,
  data,
  filename = 'image.scr'
}) {
...
function combineTapImages(ctx, ...taps) {
  let length = 0;
...
function generateROMImage({
  ctx,
  data,
  filename
}) {
  const loader = generateAutoLoaderForScreen({
    ctx
  });
  const screen = generateScreenBlock({
    ctx,
    data,
    filename
  });
...
function generateROMImageAsTap({
  ctx,
  data
}) {
...
function headerAsBytes({
  ...
  return new Uint8Array([filetype, ...(0, _audioConsts.encode)(filename.padEnd(10, ' ').slice(0, 10)), // Name (filename 10 chars padded with white space)
  ...(0, _audioConsts.byteAsWord)(length), ...(0, _audioConsts.byteAsWord)(param1), ...(0, _audioConsts.byteAsWord)(param2)]);
}
},{"./audio-consts.js":"lib/audio/audio-consts.js"}],"lib/audio/make-wav.js":[function(require,module,exports) {
"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.createWavFromBuffer = createWavFromBuffer;
exports.convertBlock = convertBlock;
exports.default = void 0;

// via https://stackoverflow.com/a/43747465
class Wav {
...
exports.loadGist = loadGist;

async function loadGist() {
  const id = getGistId(window.location.toString());
  const res = await fetch(`https://api.github.com/gists/${id}`);
  const json = await res.json();
  const file = Object.keys(json.files).map(key => json.files[key]).find(_ => _.filename.toLowerCase().endsWith('.bas'));
...
},{}],"bas/index.js":[function(require,module,exports) {
"use strict";
...
require("./basic-syntax.js");

var _save = _interopRequireDefault(require("../lib/save.js"));

var _lib = require("../lib/unpack/lib.js");

var _index = require("../lib/audio/index.js");
...
cm.on('keydown', (cm, event) => {
  ...
  if (event.key === 'Enter') {
    ...
    const newLine = (0, _txt2bas.line2bas)(line);
    ...
    try {
      ...
      cm.eachLine(({
        ...
        if (text.trim().length > 0) {
          const data = (0, _txt2bas.line2bas)(text);

          if (data.lineNumber === newLine.lineNumber) {
            inserted = true; // >1 because all lines include 0x0d
      ...
      const res = (0, _txt2bas.line2txt)(basic);
      cm.setValue(res);
      const scroll = cm.getScrollerElement().scrollTop; // attempt to put the cursor back in the best spot
...
function download(action) {
  ...
  */
  ...
  if (action === 'wav') {
    const basic = (0, _txt2bas.file2bas)(cm.getValue(), action, filename, false);
    const param1 = new DataView(basic.buffer).getUint16(0, false); // ?

    const res = (0, _index.generateBlock)({
      type: 'PROGRAM',
    ...
    const wav = (0, _makeWav.createWavFromBuffer)(res.getChannelData(0), 44100);
    const bufferLists = [];
    ...
    (0, _save.default)(bufferLists, filename + '.wav');
    cm.setValue((0, _txt2bas.line2txt)(basic));
  } else {
    let file = (0, _txt2bas.file2bas)(cm.getValue(), action, filename);
    (0, _save.default)(file, filename + (action === '3dos' ? '.bas' : '.tap'));
    cm.setValue((0, _txt2bas.file2txt)(file));
  }
}

_cm.default.commands.save = () => download('3dos');

(0, _dnd.default)(document.body, file => {
  if (file[0] === 0x13) {
    console.log('decode from tap');
    cm.setValue((0, _txt2bas.file2txt)(file, 'tap'));
  } else if (file[0] === 0x50) {
    cm.setValue((0, _txt2bas.file2txt)(file, '3dos'));
  } else {
    cm.setValue((0, _lib.decode)(file));
  }
...
if (window.location.search) {
  (0, _gist.loadGist)().then(file => {
    if (file) cm.setValue(file);
  });
}
},{"../lib/dnd.js":"lib/dnd.js","../lib/$.js":"lib/$.js","../lib/cm.js":"lib/cm.js","txt2bas":"../node_modules/txt2bas/dist/index.js","./basic-syntax.js":"bas/basic-syntax.js","../lib/save.js":"lib/save.js","../lib/unpack/lib.js":"lib/unpack/lib.js","../lib/audio/index.js":"lib/audio/index.js","../lib/audio/make-wav.js":"lib/audio/make-wav.js","../lib/gist.js":"lib/gist.js"}],"../node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
var global = arguments[3];
...
},{}]},{},["../node_modules/parcel-bundler/src/builtins/hmr-runtime.js","bas/index.js"], null)
//# sourceMappingURL=/bas.d0f817fe.js.map